...
import { toJSONRPCResponse, handleJSONRPCResponse } from '../../src/server/express/rpc.js';
...
import { Session } from '../../src/server/express/types.js';
// applyDefaults();
describe('Express Server', () => {
    ...
    describe('rpc.ts - handleJSONRPCResponse', () => {
        it('should return plain JSON for agentcard/get success', async () => {
            ...
            await handleJSONRPCResponse(res as unknown as Response, 'req-1', 'agentcard/get', response);
        ...
        it('should return JSON-RPC response for non-agentcard success', async () => {
            ...
            await handleJSONRPCResponse(res as unknown as Response, 'req-1', 'message/send', response);
        ...
        it('should stream SSE for stream type', async () => {
            ...
            await handleJSONRPCResponse(res as unknown as Response, 'req-1', 'message/stream', response);
        ...
        it('should throw INTERNAL_ERROR for unknown response type', async () => {
            ...
            await expect(
                handleJSONRPCResponse(res as unknown as Response, 'req-1', 'message/send', response),
    ...
    describe('agent-request.ts', () => {
        ...
        describe('request', () => {
            it('should call next with error when agentId is missing', async () => {
                ...
                expect(error.message).toMatch(/agentId is required|Request payload validation error/);
            });

            it('should call handler with agentId from params', async () => {
                const mockHandler = jest.fn<agentRequest.Mount['handler']>();
                const req = createMockRequest({
    ...
    describe('deploy-request.ts', () => {
        ...
        describe('request', () => {
            it('should add requestId to context', async () => {
                const mockHandler = jest.fn<deployRequest.Mount['handler']>();
                const req = createMockRequest({
                    body: {},
                ...
                await deployRequest.request({
                    ...
                    context,
                    handler: mockHandler as deployRequest.Mount['handler'],
                    user: jest.fn() as any,
                });
    ...
    describe('test-request.ts', () => {
        ...
        describe('request', () => {
            it('should generate unique agentId for test', async () => {
                const mockHandler = jest.fn<testRequest.Mount['handler']>();
                const req = createMockRequest({
                    body: {},
                });
                const res = createMockResponse();
                ...
                await testRequest.request({
                    ...
                    context,
                    handler: mockHandler as testRequest.Mount['handler'],
                    user: jest.fn() as any,
                });
    ...
    describe('server.ts - fleet', () => {
        ...
        it('should apply auth middleware when provided', () => {
            const mockAuth = jest.fn((req: Request, res: Response, next: NextFunction) => next());
            const mockGet: RequestAgentRoute['implementation'] = jest.fn();
            const mockSet: CreateAgentRoute['implementation'] = jest.fn();
    ...
    describe('Integration tests', () => {
        ...
        it('should return 404 for unknown routes', async () => {
            ...
            const mockSet: CreateAgentRoute['implementation'] = jest.fn(() => Promise.resolve({ uri: 'new-agent' }));
        ...
        it('should deploy an agent and retrieve its card', async () => {
            ...
            expect(agentId).toBe('integration-test');
            // Retrieve agent card
            const cardResponse = await request(app).get(`/funny/${agentId}/.well-known/agent-card.json`);
        ...
        it('should return error for non-existent agent', async () => {
            ...
            const response = await request(app).get('/agentId/non-existent-agent/.well-known/agent-card.json');
        ...
        it('should route agent card requests through the retrieve handler', async () => {
            ...
            const response = await request(app).get('/agentId/test-agent/.well-known/agent-card.json');
