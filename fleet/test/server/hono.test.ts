...
import { handleJSONRPCResponse } from '../../src/server/hono/rpc.js';
import { toJSONRPCResponse } from '../../src/utils.js';
import { generateRequestId } from '../../src/server/hono/utils.js';
import { ResultOrError } from '../../src/types.js';
import { RequestAgentRoute, TestAgentRoute } from '../../src/routes/request/types/definitions.js';
import { CreateAgentRoute } from '../../src/routes/create/index.js';
import { createMockContext, createValidAgentConfig, MockStore } from '../mock.js';
...
import { ContextKind } from '@artinet/types';
// applyDefaults();
/**
...
const createMockHonoContext = (
    ...
    const ctx = {
        req: {
            ...
            text: jest.fn(() => Promise.resolve(JSON.stringify(overrides.body ?? {}))),
...
describe('Hono Server', () => {
    ...
    describe('rpc.ts - toJSONRPCResponse', () => {
        ...
        it('should throw error for stream type', () => {
            ...
            expect(() => toJSONRPCResponse('req-1', resultOrError)).toThrow('Invalid response type');
        });
    });
    ...
    describe('rpc.ts - handleJSONRPCResponse', () => {
        ...
        it('should throw INTERNAL_ERROR for unknown response type', async () => {
            ...
            await expect(handleJSONRPCResponse(ctx, 'req-1', 'message/send', response)).rejects.toThrow(
                /Internal error/,
            );
        });
    });
    ...
    describe('agent-request.ts', () => {
        describe('handle', () => {
            it('should transform agent-card.json path to agentcard/get request', async () => {
                ...
                try {
                    await agentRequest.handle({ session: { ctx, next }, context }, mockHandler);
                } catch (e) {
        ...
        describe('factory', () => {
            it('should create a handler with the provided implementation', async () => {
                ...
                const handler = agentRequest.factory({ implementation: mockImpl });
        ...
        describe('request', () => {
            it('should throw error when agentId is missing', async () => {
                ...
                await expect(
                    agentRequest.request({
                        ...
                        handler: jest.fn() as unknown as agentRequest.Mount['handler'],
            ...
            it('should call handler with agentId from params', async () => {
                const mockHandler = jest.fn() as unknown as agentRequest.Mount['handler'];
                const { ctx } = createMockHonoContext({
                ...
                expect(mockHandler).toHaveBeenCalledWith(
                    {
                        session: { ctx, next },
                        context: expect.objectContaining({ agentId: 'my-agent' }),
            ...
            it('should use deploymentId param as agentId', async () => {
                ...
                await agentRequest.request({
                    session: { ctx, next },
                    context: createMockContext(),
                    handler: mockHandler as agentRequest.Mount['handler'],
                    user: jest.fn(() => Promise.resolve('test-user')),
                });
                ...
                expect(mockHandler).toHaveBeenCalledWith(
                    {
                        session: { ctx, next },
                        context: expect.objectContaining({ agentId: 'deployment-123' }),
                        intercepts: undefined,
                    },
                    expect.any(Function),
                );
    ...
    describe('deploy-request.ts', () => {
        describe('handle', () => {
            it('should parse request body and call deploy', async () => {
                ...
                await deployRequest.handle({ session: { ctx, next }, context, intercepts: undefined }, mockDeploy);

                expect(mockDeploy).toHaveBeenCalledWith({ config }, context, undefined);
        ...
        describe('factory', () => {
            it('should create a handler with the provided implementation', () => {
                ...
                const handler = deployRequest.factory({ implementation: mockImpl });
        ...
        describe('request', () => {
            it('should add requestId to context from header', async () => {
                const mockHandler = jest.fn() as unknown as deployRequest.Mount['handler'];
                const { ctx } = createMockHonoContext({
                    body: {},
                ...
                expect(mockHandler).toHaveBeenCalledWith(
                    {
                        session: { ctx, next },
                        context: expect.objectContaining({ requestId: 'deploy-req-123' }),
                        intercepts: undefined,
                    },
                    expect.any(Function),
                );
    ...
    describe('test-request.ts', () => {
        describe('handle', () => {
            it('should parse test request and call test handler', async () => {
                ...
                try {
                    await testRequest.handle({ session: { ctx, next }, context }, mockTest);
                } catch (e) {
                    // handleJSONRPCResponse throws after processing
                }
        ...
        describe('factory', () => {
            it('should create a handler with the provided implementation', () => {
                ...
                const handler = testRequest.factory({ implementation: mockImpl });
        ...
        describe('request', () => {
            it('should generate unique agentId for test', async () => {
                const mockHandler = jest.fn() as unknown as testRequest.Mount['handler'];
                const { ctx } = createMockHonoContext({
                    body: {},
                });
                const next = jest.fn() as unknown as Next;
    ...
    describe('Integration tests', () => {
        ...
        it('should deploy an agent and retrieve its card', async () => {
            ...
            // Retrieve agent card
            const cardRes = await app.request(`/funny/${agentId}/.well-known/agent-card.json`);
        ...
        it('should return error for non-existent agent', async () => {
            ...
            const res = await app.request('/agentId/non-existent-agent/.well-known/agent-card.json');
        ...
        it('should route agent card requests through the retrieve handler', async () => {
            ...
            const res = await app.request('/agentId/test-agent/.well-known/agent-card.json');
            expect(res.status).toBe(200);
            expect(mockGet).toHaveBeenCalled();
            // Verify the handler received correct method
            const callArgs = (mockGet as jest.MockedFunction<typeof mockGet>).mock.calls[0] as unknown[];
        ...
        it('should pass intercepts to the handler', async () => {
            const storage = new InMemoryStore();
            const mockGet: RequestAgentRoute['implementation'] = jest.fn((request, context, intercepts) => {
                expect(intercepts).toBeDefined();
                expect((intercepts as RequestAgentRoute['intercept'][])?.length).toBe(2);
            ...
            const { app } = fleet(
                {
                    ...
                    load: loadAgent,
                    invoke: invokeAgent,
                    middleware: new Middleware().request(requestSpy).response(responseSpy),
                },
                {},
            );

            const response = await app.request('/agentId/test-agent/.well-known/agent-card.json');
