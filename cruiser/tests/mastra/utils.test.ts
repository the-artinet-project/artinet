...
const createMockAgent = (overrides: {
  ...
  getInstructions: jest.fn<() => Promise<string | { content: string }[] | { content: string }>>().mockResolvedValue(
    overrides.instructions ?? "Test instructions"
  ),
  getTools: jest.fn<() => Promise<Record<string, { description?: string }>>>().mockResolvedValue(
    overrides.tools ?? {}
  ),
});
...
describe("utils", () => {
  ...
  describe("convertToCoreMessage", () => {
    ...
    it("should throw error for data parts", () => {
      ...
      expect(() => convertToCoreMessage(a2aMessage)).toThrow(
