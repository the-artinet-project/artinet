...
export const GroupSchema = z.object({
    id: z.string().describe("Group identifier (e.g., 'team:backend', 'project:api-v2')"),
    properties: z
...
export const AgentDefinitionSchema = Runtime.AgentInfoSchema.partial({
    ...
    .extend({
        schemaVersion: z.literal('0.1.0').describe('The schema version of the agent definition'),
         ...
         */
        toolUris: z.array(z.string()).optional().describe('List of tool ids that this agent can use'),
         ...
         */
        agentUris: z.array(z.string()).optional().describe('The agent ids that this agent can call'),
...
export const AgentConfigurationSchema = AgentDefinitionSchema.extend({
     ...
     * @example
     * [
     *   { type: "mcp", uri: "filesystem", url: "http://localhost:3000/mcp/fs" },
     *   { type: "a2a", uri: "researcher", url: "https://agents.example.com/research" }
     *   { type: "a2a", uri: "local-agent-id", parameters: { name: "John Doe" } }
     * ]
     */
    services: z.array(Runtime.ServiceSchema).default([]).describe('List of MCP or A2A services'),
