...
export interface CreateModelParams
    extends
        Omit<Partial<sdk.A2A.AgentCard & { uri: string }>, 'provider' | 'skills'>,
    ...
    /**
     * Storage for the callable services.
     * This can be used as cold storage for the callable services collection.
     */
    callableStorage?: sdk.core.Manager<Callable.Agent | Callable.Tool>;
}
 ...
 * ```
 */
export class Model extends Manager implements sdk.core.Service<ServiceRequest, API.ConnectResponse> {
    /**
    ...
    protected constructor(
        ...
        super(_data, true, _params.callableStorage);
    }
     ...
     * ```
     */
    async execute({ request, options }: ServiceRequest): Promise<API.ConnectResponse> {
        if (this._addPromise) {
        ...
        const callback: (data: Callable.Response) => void =
            options?.callback ?? ((data: Callable.Response) => this.events.emit('update', data, undefined));

        return await this._react(request, this.provider, this.call.bind(this), this._history, {
            ...options,
    ...
    get engine(): sdk.A2A.Engine {
        ...
        return async function* (context: sdk.A2A.Context) {
            const message: string | undefined = sdk.extractTextContent(context.userMessage);
            ...
            const request: API.ConnectRequest = util.request(self.modelId, messages, {});
    ...
    get agent(): sdk.Agent {
        return sdk.createAgent({
            agentCard: {
                ...util.createCard(this._params.uri ?? this._params.name ?? this.modelId, this.values),
    ...
    add(service: util.CallableService, uri?: string): Model {
        if (service instanceof sdk.Service || (typeof service === 'object' && 'engine' in service)) {
        ...
        this.addPromise(
            this._add(service, uri)
                ...
                .catch((error) => {
                    logger.error(`[Model:add]: error adding service: ${error.message}`, error);
                }),
        );
        return this;
    }
     ...
     */
    async connect(
        messages: string | API.Message | API.Session | Omit<API.ConnectRequest, 'options'>,
        options?: API.ConnectOptions & {
            parentTaskId?: string;
            abortSignal?: AbortSignal;
        },
    ): Promise<string> {
        const request: API.ConnectRequest = util.request(this.modelId, messages, await util.options([], options));
