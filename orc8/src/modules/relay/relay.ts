...
export class Relay extends Manager implements Registry {
     ...
     * ```
     */
    async getAgent(id: string): Promise<sdk.Agent | sdk.AgentMessenger | undefined> {
     ...
     * ```
     */
    async registerAgent(params: sdk.Agent | sdk.AgentMessenger | sdk.MessengerParams): Promise<sdk.A2A.AgentCard> {
        ...
        const callable: Callable.Agent = Callable.Agent.from(agent, (await agent.getAgentCard()).name);
     ...
     * ```
     */
    async sendMessage({ agentId, messageSendParams }: SendMessageRequest): Promise<sdk.A2A.SendMessageSuccessResult> {
        const agent = await this.getAgent(agentId);
        if (!agent) {
            throw new Error(`Agent ${agentId} not found`);
        }
        return await (agent as Callable.AbortableSender).sendMessage(messageSendParams, {
     ...
     * ```
     */
    async cancelTask({ agentId, taskId }: CancelTaskRequest): Promise<sdk.A2A.Task> {
        const agent = await this.getAgent(agentId);
        if (!agent) {
            throw new Error(`Agent ${agentId} not found`);
        }
    ...
    async searchAgents({ query }: SearchRequest): Promise<sdk.A2A.AgentCard[]> {
        ...
        const results = await this.search(searchQuery, async (callable: CallableAgent | CallableTool) => {
            if (!(callable instanceof Callable.Agent)) {
                return false;
            }
            const agentCard = await callable.getInfo();
            return (
                ...
                agentCard.skills.some(
                    (skill: sdk.A2A.AgentSkill) =>
                        ...
                        skill.tags.some((tag: string) => tag.toLowerCase().includes(searchQuery)),
                )
            );
        });
        ...
        return (
            (
                await Promise.all(
                    results.map(async (callable: CallableAgent | CallableTool) => {
                        if (!(callable instanceof Callable.Agent)) {
                            return undefined;
                        }
                        return await callable.getInfo();
            ...
            )?.filter((info): info is sdk.A2A.AgentCard => info !== undefined) ?? []
        );
    }
