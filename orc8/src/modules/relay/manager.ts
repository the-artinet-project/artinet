...
const refreshCache = async (callables: Callable.Agent[], infos: Map<string, Runtime.AgentInfo>): Promise<void> => {
    await Promise.all(
...
const updateCache = async (callables: Callable.Agent[], infos: Map<string, Runtime.AgentInfo>): Promise<void> => {
    await Promise.all(
        callables
...
export class Manager extends Callable.Manager {
    ...
    constructor(
        agents: Map<string, Agent | AgentMessenger> = new Map(),
        throwOnSet?: boolean,
     ...
     * @returns Array of {@link A2A.AgentCard} agent cards
     */
