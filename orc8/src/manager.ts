...
export class Manager extends ResourceManager<Callable.Agent | Callable.Tool> {
    ...
    async call({
        ...
        const limit = pLimit(Math.min(Callable.DEFAULT_CONCURRENCY, request.length));
        await Promise.allSettled(
            request.map((req) => {
                return limit(async () => {
                    const result = await this._processRequest(req, options).catch((err) => {
                        logger.error(`[Manager:execute:callable:${req.uri}]: error processing request: `, err);
                        return undefined;
                    });
                    if (result) {
